...
static void ssd1306_write_byte(uint8_t chData, uint8_t chCmd) 
{
	...
	while(HAL_I2C_Master_Transmit(&hi2c1,OLED_ADDRESS,data,2,2)!=HAL_OK);
...
void ssd1306_clear_screen_from_position(uint8_t x, uint8_t y) {
    // Giả sử s_chDispalyBuffer là một mảng 2D đại diện cho bộ đệm hiển thị
    // với kích thước 128x8 (128 cột và 8 hàng)
    for (uint8_t i = y; i < 8; i++) {
        for (uint8_t j = x; j < 128; j++) {
